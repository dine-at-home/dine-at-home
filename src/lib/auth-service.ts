...
class AuthService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (result.code === 'INVALID_OTP') {
          errorMessage = 'Invalid or expired OTP. Please check the code and try again, or request a new one.';
        } else if (result.code === 'BAD_REQUEST') {
   ...
   */
  async getGoogleAuthUrl(): Promise<{ success: boolean; data?: { authUrl: string }; error?: string }> {
    try {
   ...
   * Update user role
   */
  async updateRole(role: 'guest' | 'host' | 'admin'): Promise<{ success: boolean; error?: string }> {
    const token = this.getToken();
    if (!token) {
